...
Для слияния веток в Git используется команда *git merge branch_name*. Если возникает конфликт версий - вручную редактируем, удаляя сервисные сообщения. После чего сохраняем состояние, делая коммит.
## Удаление веток
Для удаления ненужной ветки используется команда *git branch -d branch_name* параметр -d даёт команду на удаление ветки