...
Для слияния веток используеться команда *git merge <Название ветки>*. Если будут конфликты то, будет предложено их совместить, что нужно сделать в ручную. После исправления нужно сделать коммит к итоговому файлую.

## Удаление веток
Если мы понимаем, что ветка больше не нужна, можно ее удалить.  При этом удаляемая ветка не должна быть той веткой, в которой мы находимся. Удалим ветку calc: *git branch -d <название ветки>*.

## Удаленные репозитории
1. Для клонирования удаленного репозитория используеться команда *git clone <ссылка на репозиторий>*. 

2. Для отпраки изменений в удаленный репозиторий используеться команда *git push -u origin main*.