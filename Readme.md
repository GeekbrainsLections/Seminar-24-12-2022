...
Для подключения к удаленному репозиторию, необходимо выполнить команду *git clone <ссылка на репозиторий>*. Ссылку можно найти нажав на кнопку **Code**.

Для работы с чужим репозиторием необходимо сделать *fork* репозитория после чего подключиться, с помощью команды указанной выше. После подключения следует перейти в дирректорию с файлом с помощью команды *cd <имя файла>*

Если вы хотите отправить измененный репозиторий создателю, вам следует выполнить *pull request*. Автору репозитория может посмотреть что вы изменили в файле. Автор может либо добавить комит, либо принять изменения, либо запросить переделать работу. 
