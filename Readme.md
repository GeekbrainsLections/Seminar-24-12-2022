...
## Журнал изменений
Для просмотра журнала изменений используется команда *git log*. Для этого в терминале с папкой-репозиторием необходимо написать *git log*. По умолчанию (без аргументов) *git log* перечисляет коммиты сделанные в репозитории обратном к хронологическому порядке - последние коммиты находятся наверху. Команда *git log* имеет очень большое количество опций для поиска коммитов по разным критериям.  
Например, команда *git log -p* или *git log --patch* показывает разницу, внесенную в каждый коммит; команда *git log --start* показывает сокращенную статистику для каждого коммита, т.е. печатает под каждым из коммитов список и количество измененных файлов, а также сколько строк в каждом из файлов было добавлено и удалено; команда *git log --oneline* выводит каждый коммит в отдельную строку, что может быть удобным при просматривании большого количества коммитов; команда *git log --oneline --all --graph* показывает все коммиты и ветки с историей слияний.
...
Для создания новой ветки используется команда *git branch*. Для этого в терминале с папкой-репозиторием необходимо написать *git branch <название ветки>*. Эта команда только создает ветку, но не изменяет указатель на текущую ветку HEAD.
